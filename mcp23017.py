...
class mcp23017():
	...
	def set_gpio_high(self, bank, target_gpio, status = None):
		#print ("set_high address = ",self.address," bank = ",bank,"target = ",target_gpio)
		if(status == None):
	...
	def set_gpio_low(self, bank, target_gpio, status = None):
		#print ("set_low address = ",self.address," bank = ",bank,"target = ",target_gpio)
		if(status == None):
			status = self.get_gpio_bank_status(bank)
		new_status_byte = status & (~(1 << target_gpio))
		self.smbus.write_byte_data(self.address, MCP23017_REG_GPIOA+bank-1, new_status_byte)
	...
	def set_gpio_low_by_index(self, target_gpio):
		...
		while target_gpio > 7:
			target_gpio = target_gpio - 8
			bank = bank + 1
		self.set_gpio_low(bank, target_gpio)
	...
	def get_single_gpio_status(self, bank, target_gpio):
		#print ("bank =",bank," target_gpio = ",target_gpio)
		status = self.get_gpio_bank_status(bank)
		#print ("bank status = ",status)
		target_byte_value = 1 << target_gpio
		tstatus = (status & target_byte_value) != 0
		#print ("tstatus =",tstatus)
		return tstatus

	def get_single_gpio_status_by_index(self, target_gpio):
		#print("single target = ",target_gpio) 
		bank = 1
		while target_gpio > 7:
			target_gpio = target_gpio - 8
			bank = bank + 1
		#print("single bank = ",bank,"target = ",target_gpio)     
		return self.get_single_gpio_status(bank, target_gpio)
